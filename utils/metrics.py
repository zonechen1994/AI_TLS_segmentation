...
class Evaluation_metrics():
    ...
    def cal_total_metrics(self, pred, mask):
        # MAE
        iou = torch.mean(self._get_iou(pred, mask)).item()
        dice = torch.mean(self._get_dice(pred, mask)).item()
        ...
        return iou, dice, mae, max_f, avg_f, s_score
    ...
    def _get_iou(self, pred, target, size_avarage=True):
        b = pred.shape[0]
        IoU = 0.0
        for i in range(0,b):
    ...
    def _get_dice(self, pred, target):
        smooth = 1
